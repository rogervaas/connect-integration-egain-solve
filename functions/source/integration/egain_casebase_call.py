...
def lambda_handler(event, context):
    ...
    stored_data = 'null'
    if 'storedData' in event['Details']['ContactData']['Attributes']:
        stored_data = event['Details']['ContactData']['Attributes']['storedData']
    ivr_stage = 'MAIN'
    if 'IVRStage' in event['Details']['ContactData']['Attributes']:
    ...
    if len(stored_data) > 0 and stored_data != 'null':
        ...
        try:
            if 'Q1' in stored_data['qn']:
                data = data + "&" + stored_data['qn'] + "=" + str(stored_data['ans' + str(ivr_stage)])
            elif 'Q' in stored_data['qn']:
                data = data + "&" + stored_data['qn'] + "=" + str(ivr_stage)
